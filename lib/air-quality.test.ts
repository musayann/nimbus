...
describe('fetchAirQuality', () => {
  it('returns air quality data from nearest station', async () => {
    ...
    const result = await fetchAirQuality(-1.9403, 29.8739)
  ...
  it('rounds coordinates before haversine lookup', async () => {
    ...
    const result = await fetchAirQuality(-1.9467, 29.8781)
    expect(result).not.toBeNull()
  ...
  it('returns null on fetch error', async () => {
    ...
    await expect(fetchAirQuality(-1.9403, 29.8739)).rejects.toThrow()
  })

  it('returns null on non-ok upstream response', async () => {
